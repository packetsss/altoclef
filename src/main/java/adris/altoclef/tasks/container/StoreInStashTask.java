...
public class StoreInStashTask extends Task {
    ...
    protected void onStart(AltoClef mod) {
        if (storedItems == null) {
            storedItems = new ContainerStoredTracker(slot -> {
                ...
                return currentContainer.isPresent() && stashRange.contains(currentContainer.get());
            });
        }
    ...
    protected Task onTick(AltoClef mod) {
        ...
        Predicate<BlockPos> validContainer = blockPos -> {
            if (!stashRange.contains(blockPos))
                return false;
        ...
        if (mod.getBlockScanner().anyFound(validContainer, TO_SCAN)) {
            ...
            return new DoToClosestBlockTask(
                    (BlockPos bpos) -> new StoreInContainerTask(bpos, false, storedItems.getUnstoredItemTargetsYouCanStore(mod, toStore)),
                    validContainer,
                    TO_SCAN
            );
        }
    ...
    public boolean isFinished(AltoClef mod) {
        return storedItems != null && storedItems.getUnstoredItemTargetsYouCanStore(mod, toStore).length == 0;
    }

    @Override
    protected boolean isEqual(Task other) {
        if (other instanceof StoreInStashTask task) {
            return task.stashRange.equals(stashRange) && task.getIfNotPresent == getIfNotPresent && Arrays.equals(task.toStore, toStore);
        }
        return false;
    }
